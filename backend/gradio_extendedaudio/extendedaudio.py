...
class WaveformOptions:
    """
    A dataclass for specifying options for the waveform display in the ExtendedAudio component. An instance of this class can be passed into the `waveform_options` parameter of `gr.Audio`.
    Parameters:
...
class ExtendedAudio(
    ...
    EVENTS = [
        ...
        Events.upload,
        Events.input,
        EventListener("generate", doc="User clicked Generate in speech mode.")
    ]
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: A path, URL, or [sample_rate, numpy array] tuple (sample rate in Hz, audio data as a float or int numpy array) for the default value that ExtendedAudio component is going to take. If a function is provided, the function will be called each time the app loads to set the initial value of this component.
        ...
        if self.streaming and "microphone" not in self.sources:
            raise ValueError(
                "ExtendedAudio streaming only available if sources includes 'microphone'."
            )
    ...
    def preprocess(
        ...
        if self.min_length is not None or self.max_length is not None:
            ...
            if self.min_length is not None and duration < self.min_length:
                raise Error(
                    f"ExtendedAudio is too short, and must be at least {self.min_length} seconds"
                )
            if self.max_length is not None and duration > self.max_length:
                raise Error(
                    f"ExtendedAudio is too long, and must be at most {self.max_length} seconds"
                )
    ...
    def postprocess(
        ...
        if isinstance(value, bytes):
            ...
            raise ValueError(f"Cannot process {value} as ExtendedAudio")
    ...
    def _convert_to_adts(data: bytes):
        if wasm_utils.IS_WASM:
            raise wasm_utils.WasmUnsupportedError(
                "ExtendedAudio streaming is not supported in the Wasm mode."
            )
    ...
    async def covert_to_adts(data: bytes) -> tuple[bytes, float]:
        return await anyio.to_thread.run_sync(ExtendedAudio._convert_to_adts, data)
    ...
    def check_streamable(self):
        if (
            ...
            raise ValueError(
                "ExtendedAudio streaming only available if source includes 'microphone'."
            )
