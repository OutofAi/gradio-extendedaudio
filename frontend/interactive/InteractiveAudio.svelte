<script lang="ts">
	...
	import AudioSpeech from "../speech/AudioSpeech.svelte";
	import StreamAudio from "../streaming/StreamAudio.svelte";
	import SelectSource from "./SelectSource.svelte";
	import type { WaveformOptions } from "../shared/types";

	type SpeechRequest = { mode: "speech"; text: string; voice: string };
	...
	export let sources:
		...
		| ["microphone", "upload"]
		| ["upload", "microphone"] 
		| ["upload", "microphone", "speech"] = ["microphone", "upload"];
	...
	export let active_source: "microphone" | "upload" | "speech";
	...
	const generate_dispatch = createEventDispatcher<{
		// add this line:
		generate: { text: string; choice: string };
		// (keep your existing event typings)
	}>();

	function handleGenerate(e: CustomEvent<{ text: string; choice: string }>) {
		// forward to the parent (Index.svelte) as a Svelte event
		generate_dispatch("generate", e.detail);
	}
